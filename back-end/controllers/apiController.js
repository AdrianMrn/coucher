...
exports.exportTrip = function(id, ownerid, next){
    trip_schema.findOne({_id: mongoose.Types.ObjectId(id), ownerid:ownerid}, function(err, trip){
        if (err) console.log(err);
        const options = {
            "format": 'A4',
            "border": {
        ...
        for (var i=0; i<trip.stops.length; i++) {
            ...
            route += `
            <div class="route">
                <svg id="svg_marker" class="svg_marker_` + 1 + ` marker" data-name="svg_marker" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 79.41">
                    <title>marker</title>
                ...
                <div class="info">
                    ...
                    <div>
                        ...
                        <p>` + (stop.couchName == undefined ? "None chosen" : stop.couchName) + (stop.couchUrl == undefined ? "" : " ("+ stop.couchUrl +")") + `</p>
                    </div>
                </div>
            </div>` +
            (trip.stops[i+1] ? `<div class="hitchhike">
                <div class="icon"></div>
                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 73.4 91.4" style="enable-background:new 0 0 73.4 91.4;" xml:space="preserve">
                    <g>
                        <g>
                ...
                <p>` + (trip.hitchhikingSpots[i].spotAddress == undefined ? "None chosen" : trip.hitchhikingSpots[i].spotAddress) + `</p>
            </div>
            `: "");
        }
        ...
        <html>
          ...
          <body>
            ...
            <div class="wrapper">
                <p>
                    This is an export of your trip. Make sure to check whether hitchhiking spots are safe. <br/> 
                    use hitchwiki.org to check the comments and ratings if you are unsure. <br/>
                    Also remember to <strong>confirm couches with hosts</strong>. This is not a service we provide. <br/>
                    We are not responsible if you end up in front of a closed door.
                </p>
            </div>
            <div class="wrapper">
                ` + route + `
...
exports.getTrip = function(id, ownerid, next){
    trip_schema.findOne({_id: mongoose.Types.ObjectId(id), ownerid:ownerid}, function(err, trip){
        if (err) console.log(err);
        next(trip);
    });
